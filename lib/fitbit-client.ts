...
export function getFitbitAuthURL(params: {
  ...
  const redirectUri = `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/fitbit/callback`
...
async function fetchFitbitAPI<T>(endpoint: string, accessToken: string): Promise<T> {
...
export interface FitbitHeartRateResponse {
  'activities-heart': Array<{
    dateTime: string
    value: {
      customHeartRateZones: Array<{ name: string; min: number; max: number; minutes: number; caloriesOut: number }>
...
export async function getHeartRate(accessToken: string, date: Date = new Date()): Promise<FitbitHeartRateResponse> {
  return fetchFitbitAPI(`/1/user/-/activities/heart/date/${formatDate(date)}/1d.json`, accessToken)
}

// Fetch daily activity summary
export async function getActivitySummary(accessToken: string, date: Date = new Date()): Promise<FitbitActivityResponse> {
  return fetchFitbitAPI(`/1/user/-/activities/date/${formatDate(date)}.json`, accessToken)
}

// Fetch sleep log
export async function getSleepLog(accessToken: string, date: Date = new Date()): Promise<FitbitSleepResponse> {
  return fetchFitbitAPI(`/1.2/user/-/sleep/date/${formatDate(date)}.json`, accessToken)
}

// Fetch SpO2 data
export async function getSpO2(accessToken: string, date: Date = new Date()): Promise<FitbitSpO2Response> {
  return fetchFitbitAPI(`/1/user/-/spo2/date/${formatDate(date)}.json`, accessToken)
}

// Fetch breathing rate
export async function getBreathingRate(accessToken: string, date: Date = new Date()): Promise<FitbitBreathingRateResponse> {
  return fetchFitbitAPI(`/1/user/-/br/date/${formatDate(date)}.json`, accessToken)
}

// Fetch user profile
export async function getProfile(accessToken: string): Promise<FitbitProfileResponse> {
  return fetchFitbitAPI('/1/user/-/profile.json', accessToken)
}

// Fetch all health data for a date (combined call)
export async function getAllHealthData(accessToken: string, date: Date = new Date()) {
